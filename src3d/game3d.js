...
const REP_IDLE=["Got the new drill in the truck.","This plate? Titanium. GORGEOUS.","Can I scrub in? I basically work here.",
  ...
  "I've watched like 400 of these.","Doctor, have you SEEN the new mallet?","Doc, one sec — new catalog."];
const REP_BOOST=["Hell yeah bro!","STRYKER STRONG!","Hydrate, king.","Let's GOOO!"];
const REP_MALLET=["Try the new one. It's TITANIUM.","Demo unit. Keep it quiet.","This one's off the truck.","Latest gen. You've earned it."];
const REP_CALM=["Doctor. Breathe. Think of the demo.","Easy, doc — lunch is on Stryker.","Doctor, the catalog. Focus on the catalog.","We're fine. New hips Friday, remember?"];
const REP_CALM_R=200;
...
const GL2=renderer.capabilities.isWebGL2; // NPOT mipmaps need WebGL2
...
steelTex.anisotropy=Math.min(8,MAXANISO);
...
/* lights — a real OR is bright, even, and slightly cold; keep one struct so
   init/blackout restores can't drift out of sync */
const LIT={hemi:.44,dir:.24,room:.7,spot:1.1};
const hemiL=new THREE.HemisphereLight(0xecf7f2, 0x3a544e, LIT.hemi); scene.add(hemiL);
const dirL=new THREE.DirectionalLight(0xf4fffa, LIT.dir); dirL.position.set(0.4,1,0.3); scene.add(dirL);
const roomLights=[], spots=[];
[[600,375],[1800,375],[600,1125],[1800,1125]].forEach(p=>{
  const L=new THREE.PointLight(0xf4fcf8, LIT.room, 1500, 1.6); L.position.set(p[0],CEIL-14,p[1]);
...
[[600,380],[1800,380]].forEach(p=>{
  const s=new THREE.SpotLight(0xfdfbf0, LIT.spot, 900, 0.72, 0.55, 1.2);
  ...
  // laminar-flow canopy: the perforated air diffuser array every real OR hangs over the table
  const lfFrame=new THREE.Mesh(new THREE.BoxGeometry(390,8,300), mat('#dfe8e6'));
  lfFrame.position.set(p[0],CEIL-8,p[1]); scene.add(lfFrame);
  if(!scene.userData.lfTex){
    const lc=document.createElement('canvas'); lc.width=lc.height=256;
    const lcx=lc.getContext('2d'); lcx.fillStyle='#eef4f1'; lcx.fillRect(0,0,256,256);
    lcx.strokeStyle='rgba(120,140,135,.55)'; lcx.lineWidth=1; lcx.beginPath();
    for(let i=0;i<=256;i+=8){ lcx.moveTo(i,0); lcx.lineTo(i,256); lcx.moveTo(0,i); lcx.lineTo(256,i); }
    lcx.stroke();
    const lt=new THREE.CanvasTexture(lc); lt.encoding=THREE.sRGBEncoding;
    lt.wrapS=lt.wrapT=THREE.RepeatWrapping; lt.repeat.set(6,4.5);
    scene.userData.lfTex=lt;
  }
  const lfPanel=new THREE.Mesh(new THREE.PlaneGeometry(370,280),
    new THREE.MeshBasicMaterial({map:scene.userData.lfTex}));
  lfPanel.rotation.x=Math.PI/2; lfPanel.position.set(p[0],CEIL-12.5,p[1]); scene.add(lfPanel);
  for(const e of [[-190,0,4,300],[190,0,4,300],[0,-150,390,4],[0,150,390,4]]){
    const strip=new THREE.Mesh(new THREE.BoxGeometry(e[2],3,e[3]), bmat('#f8fffb'));
    strip.position.set(p[0]+e[0],CEIL-11,p[1]+e[1]); scene.add(strip); } });
const dusts=[];
...
if(GL2){ floorTex.minFilter=THREE.LinearMipmapLinearFilter; floorTex.generateMipmaps=true; }
else { floorTex.minFilter=THREE.LinearFilter; floorTex.generateMipmaps=false; }
...
{ const fl=new THREE.Mesh(new THREE.PlaneGeometry(W,H),
    new THREE.MeshStandardMaterial({map:floorTex,roughness:.3,metalness:.06,envMapIntensity:.6}));
  ...
  if(GL2){ ceilTex.minFilter=THREE.LinearMipmapLinearFilter; ceilTex.generateMipmaps=true; }
  else { ceilTex.minFilter=THREE.LinearFilter; ceilTex.generateMipmaps=false; }
...
const wallMeshes=[];
for(const r of walls){ const m=new THREE.Mesh(new THREE.BoxGeometry(r.w,140,r.h), mat('#4d7c75'));
  ...
  const upper=new THREE.Mesh(new THREE.BoxGeometry(r.w+1,50,r.h+1), mat('#6d9a92'));
...
/* photo ceramic-tile wainscot on the walls, one texture repeat per 140 units of run.
   the source photo has a ragged splice band mid-image — crop to the clean upper region */
loadTex('wall','textures/wall.jpg',im=>{
  const clean=Math.round(im.height*.47);
  const cvs=document.createElement('canvas'); cvs.width=cvs.height=1024;
  cvs.getContext('2d').drawImage(im,0,0,im.width,clean,0,0,1024,1024);
  for(const m of wallMeshes){
    ...
    t.repeat.set(m.userData.wallLen/140,1);
    m.material=new THREE.MeshStandardMaterial({color:0xb2d2ca,map:t,roughness:.32,metalness:.03,envMapIntensity:.5});
  }
});

/* static deco: PACU curtains, door frames w/ X-ray lights, wet floor sign */
const CLOSET={x:2040,y:1440};   // where he takes them
let closetDoor=null;
{ const deco=new THREE.Group();
  ...
  // the janitor closet — south wall of PACU. he keeps his mops in there. and, eventually, you.
  { const hdr=new THREE.Mesh(new THREE.BoxGeometry(84,24,10), mat('#dfe6e4'));
    hdr.position.set(CLOSET.x,122,1470); deco.add(hdr);
    for(const s of [-1,1]){ const j=new THREE.Mesh(new THREE.BoxGeometry(10,112,10), mat('#dfe6e4'));
      j.position.set(CLOSET.x+s*38,56,1470); deco.add(j); }
    closetDoor=new THREE.Group(); closetDoor.position.set(CLOSET.x-33,0,1468); deco.add(closetDoor);
    const slab=new THREE.Mesh(new THREE.BoxGeometry(66,110,5), mat('#2f4a40'));
    slab.position.set(33,55,0); slab.castShadow=HQ; closetDoor.add(slab);
    const knob=new THREE.Mesh(new THREE.SphereGeometry(2.6,10,8), smat('#c9d4d2'));
    knob.position.set(58,52,-4.5); closetDoor.add(knob);
    const sc=document.createElement('canvas'); sc.width=128; sc.height=32;
    const scx=sc.getContext('2d'); scx.fillStyle='#e8ece9'; scx.fillRect(0,0,128,32);
    scx.strokeStyle='#5a6b68'; scx.lineWidth=3; scx.strokeRect(1,1,126,30);
    scx.fillStyle='#31413d'; scx.font='900 19px Impact,sans-serif'; scx.textAlign='center';
    scx.fillText('JANITOR',64,24);
    const st=new THREE.CanvasTexture(sc); st.encoding=THREE.sRGBEncoding;
    const sign=new THREE.Mesh(new THREE.PlaneGeometry(36,9), new THREE.MeshBasicMaterial({map:st}));
    sign.rotation.y=Math.PI; sign.position.set(33,88,-2.8); closetDoor.add(sign);
  }
  scene.add(deco); }
...
function buildPropMesh(p){
  ...
  switch(p.t){
    ...
    case 'ortable':
      bx(g,36,6,30,'#3a4a52',0,3,0);
      bx(g,22,26,20,'#9aa6ac',0,19,0);
      shiny(bx(g,28,4,24,'#aab8b6',0,34,0));
      bx(g,116,8,54,'#55676f',0,52,0);
      // articulated pad sections head→foot, black like the real thing
      bx(g,20,7,44,'#22262a',-51,59,0); bx(g,34,7,46,'#22262a',-21,59,0);
      bx(g,24,7,46,'#22262a',10,59,0); bx(g,32,7,44,'#22262a',41,59,0);
      for(const s2 of [-1,1]) shiny(bx(g,102,2.5,2.5,'#aab8b6',0,56,s2*28));
      bx(g,20,6,46,'#c3d2cf',-58,60,0); break;
    ...
    case 'warmer': // stainless blanket/fluid warming cabinet with glass doors
      shiny(bx(g,58,96,48,'#b8c4c2',0,48,0));
      bx(g,46,26,2,'#d8ecf2',0,70,25,true); bx(g,46,26,2,'#d8ecf2',0,36,25,true);
      bx(g,46,2,2.4,'#8a9d99',0,58,25,true); bx(g,46,2,2.4,'#8a9d99',0,24,25,true);
      bx(g,4,4,1.5,'#ffb52e',22,88,24.5,true); break;
    ...
    case 'orlight': // dual-head articulated surgical light
      cyl(g,5,5,4,'#c9d4d2',0,CEIL-4,0);
      cyl(g,2.4,2.4,20,'#c9d4d2',0,CEIL-15,0);
      sph(g,4.5,'#aab8b6',0,CEIL-25,0);
      for(const s2 of [-1,1]){
        const arm=bx(g,30,3,3,'#c9d4d2',s2*14,CEIL-28,s2*5); arm.rotation.y=s2*.35; arm.rotation.z=s2*.28;
        cyl(g,2,2,12,'#aab8b6',s2*26,CEIL-34,s2*9);
        cyl(g,17,21,7,'#f0f4f3',s2*26,CEIL-42,s2*9,false,20);
        cyl(g,15,15,2,'#fffbe0',s2*26,CEIL-46,s2*9,true,20);
        cyl(g,2.2,3,5,'#3ec1d3',s2*26,CEIL-49,s2*9);
      } break;
    ...
    case 'sink': // stainless scrub trough with backsplash + knee panel
      shiny(bx(g,70,38,26,'#c9d4d2',0,21,0));
      bx(g,62,10,18,'#8fa5ad',0,42,1);
      shiny(bx(g,70,26,4,'#c9d4d2',0,54,-13));
      bx(g,58,10,2,'#aab8b6',0,10,13.2);
      for(const s2 of [-14,14]){ cyl(g,1.6,1.6,14,'#d8e8ee',s2,55,-8);
        bx(g,2,2,10,'#d8e8ee',s2,62,-3); } break;
    ...
    case 'mayo': cyl(g,2.5,2.5,76,'#9aa8a5',0,38,0); bx(g,34,3,26,'#dfeae7',0,78,0);
      bx(g,32,1.6,24,'#4a7cc0',0,79.6,0,true);
      bx(g,18,1.6,2.4,'#e8f0ee',0,81,3,true); break;
    ...
    case 'backtable': // sterile-draped: blue wrap over the top, instruments laid in rows
      for(const sx2 of [-38,38]) for(const sz of [-14,14]) shiny(cyl(g,2.5,2.5,52,'#aab8b6',sx2,26,sz));
      shiny(bx(g,86,4,36,'#c9d4d2',0,54,0));
      bx(g,84,12,34,'#426fae',0,48,0); bx(g,82,2,33,'#4a7cc0',0,57,0,true);
      bx(g,20,3,12,'#e8f0ee',-20,60,4,true); bx(g,16,3,10,'#e8f0ee',14,60,-6,true);
      bx(g,12,3,8,'#c9d4d2',30,60,2,true); break;
...
function buildRepMesh(o){
  ...
  mkHead(g,U,79,8.4,o.skin,o.hair,'#c0392b',null);      // the red hat
  bx(U.head,1.2,1.2,7,'#16302c',8.6,1.4,0,true);        // wraparound shades
  return g;
}
function buildJanMesh(){
  ...
  { const mop=new THREE.Group();
    ...
    mop.scale.setScalar(.78); mop.position.set(6,-2,2); U.armR.lo.add(mop); U.mop=mop; }
...
function buildPlayerBody(){
  // you, seen for the first and last time: teal scrubs to match the arm you've been staring at
  const g=new THREE.Group(); const U=g.userData;
  U.legL=mkLimb2(g,0,34,-6,17,17,4.4,'#2e8f86','#2e8f86','#2b3338',0,true);
  U.legR=mkLimb2(g,0,34,6,17,17,4.4,'#2e8f86','#2e8f86','#2b3338',0,true);
  { const t=new THREE.Mesh(new THREE.CylinderGeometry(10,12,28,14), mat('#37a196'));
    t.position.y=49; t.scale.set(1.15,1,1); g.add(t); U.torso=t; }
  U.armL=mkLimb2(g,0,60,-14,14,13,3.6,'#37a196','#f0d8c0','#f0d8c0',3.9,false);
  U.armR=mkLimb2(g,0,60,14,14,13,3.6,'#37a196','#f0d8c0','#f0d8c0',3.9,false);
  mkHead(g,U,76,8.8,'#f0d8c0','#5a4632','#2e8f86',null);
  return g;
}
...
{ const handle=cyl(mopVM,1.5,1.5,58,'#6a5a3a',0,0,-26); handle.rotation.x=Math.PI/2;
  const head=sph(mopVM,8,'#cfc8b8',0,-3,-52); head.scale.set(1,.55,1);
  sph(mopVM,4.2,'#7d1424',3.5,-3.5,-49); sph(mopVM,3,'#8d1a2c',-3,-3,-54);
  sph(mopVM,2.2,'#5d0e1c',0,-4.5,-46);
  for(let i=0;i<7;i++){ const s=cyl(mopVM,.7,.5,9,i<3?'#8d1a2c':'#bdb6a6',rand(-6,6),-9,-52+rand(-5,5));
    s.rotation.x=rand(-.25,.25); s.rotation.z=rand(-.3,.3); }
  const arm=cyl(mopVM,3.2,4.2,20,'#2e8f86',4,-6,-1); arm.rotation.x=1.15; arm.rotation.z=-.35;
  sph(mopVM,4,'#f0d8c0',0,-1,-8);
...
let knockCount=0, syrMode=false, pMalletT=0;
...
function init(){
  ...
  pMalletT=0; pmalletVM.visible=false;
  ...
  npcs=[
    ...
    mkNPC('rep',700,360,'Chad, Stryker'), mkNPC('rep',1840,470,'Tanner, Stryker'),
  ];
  npcs[6].ang=-1.0;
  npcs[7].buddy=npcs[3]; npcs[8].buddy=npcs[4];   // each rep shadows a surgeon
  npcs[3].rep=npcs[7]; npcs[4].rep=npcs[8];       // and the surgeon knows his rep is watching
  for(const n of npcs){
  ...
  hemiL.intensity=LIT.hemi; dirL.intensity=LIT.dir; if(ceilMat) ceilMat.color.setScalar(1);
  for(const s of spots) s.intensity=LIT.spot;
  for(const L of roomLights) L.intensity=LIT.room;
...
function trySwing(){
  ...
  const hasMop=mopT>0, hasPM=pMalletT>0;
  const rng=hasMop?185:hasPM?165:150, arc=hasMop?0.9:hasPM?0.72:0.62, dmg=hasMop?95:hasPM?82:38;
  player.swing=.26; player.swingCd=hasMop?.26:.32; sfxWhoosh();
  let hitAny=false;
  for(const p of props){ if(p.dead) continue;
    const px=p.x-player.x, py=p.y-player.y;
    const d=Math.hypot(px,py)-(p.r||Math.min(p.w,p.h)/2);
    if(d<rng){ const a=Math.atan2(py,px); let da=Math.abs(a-player.ang);
      da=Math.min(da,Math.PI*2-da);
      if(da<arc){ hitProp(p,dmg,player.x,player.y,hasMop?1.4:hasPM?1.3:1); hitAny=true;
        if(hasMop) burst(p.x,p.y,5,'#7d1424',180,.6);
        else if(hasPM) burst(p.x,p.y,5,'#ffd27a',200,.5); } } }
  ...
  if(!hasMop && janitor.downT<=0 && janitor.noKnock<=0){
    ...
    if(jd<150+janitor.r){ const a=Math.atan2(janitor.y-player.y,janitor.x-player.x);
      let da=Math.abs(a-player.ang); da=Math.min(da,Math.PI*2-da);
      if(da<0.62){ knockJanitor(); hitAny=true; } } }
  if(hitAny) shake=Math.max(shake,hasMop?5:hasPM?4.5:3.5);
...
function knockJanitor(){
  ...
  mopT=8; knockCount++;
  mallet.visible=false; pmalletVM.visible=false; mopVM.visible=true;
...
function update(dt){
  ...
  // the borrowed mop drips, then runs dry
  if(mopT>0){
    mopT-=dt;
    if(Math.random()<dt*4) spawnPart(P.x+rand(-8,8),30,P.y+rand(-8,8),'#7d1424',30,.7);
    if(mopT<=0){ mopVM.visible=false; syrVM.visible=false; syrMode=false;
      if(pMalletT>0) pmalletVM.visible=true; else mallet.visible=true;
      showBanner('the mop is spent'); sfxThud(); }
  }
  // the demo mallet goes back on the truck eventually
  if(pMalletT>0){
    pMalletT-=dt;
    if(pMalletT<=0){ pmalletVM.visible=false;
      if(mopT<=0){ mallet.visible=true; showBanner('demo period expired'); sfxClang(); } }
  }
  // combo decay
  ...
  const near=janitor.downT>0?0:clamp(1-jd/520,0,1);
  vigEl.style.opacity=near*.9;
  radarFill.style.width=(near*100)+'%';
  radarTxt.textContent= janitor.downT>0?'he is down. SMASH.':
    jd>800?'signal lost': jd>520?'he is walking': jd>300?'he sees you': jd>160?'RUN.':'—';
  ...
  for(const L of roomLights) L.intensity=LIT.room;
  if(near>.35) rl.intensity=LIT.room-(near-.35)*rand(.35,.85);
  ...
  if(blackoutT>0){
    ...
    if(blackoutT>0){
      ...
      hemiL.intensity=LIT.hemi; dirL.intensity=LIT.dir; ceilMat.color.setScalar(1);
      for(const s of spots) s.intensity=LIT.spot;
...
function updateNPC(n,dt){
  ...
  if(n.kind==='circ'){
    ...
    n.boostCd=(n.boostCd||0)-dt; n.malletCd=(n.malletCd||0)-dt;
    if(n.say<=0&&n.lineT<=0&&pd<420){ say(n,pick(REP_IDLE)); n.say=rand(6,10); }
    // a rep is never far from his surgeon — that's where the purchase orders live
    const d=Math.hypot(n.tx-n.x,n.ty-n.y);
    if(d<12||n.moveT<=0){ n.moveT=rand(1.5,3.5);
      if(n.buddy){ const a=rand(0,Math.PI*2), rr2=rand(40,100);
        n.tx=clamp(n.buddy.x+Math.cos(a)*rr2,60,W-60);
        n.ty=clamp(n.buddy.y+Math.sin(a)*rr2,60,H-60); }
      else { n.tx=rand(90,W-90); n.ty=rand(90,H-90); } }
    const bd=n.buddy?dist(n,n.buddy):0;
    if(bd>150){ n.tx=n.buddy.x; n.ty=n.buddy.y; }   // orbit points go stale when he sprints — chase him
    const sp3=bd>150?150:88;
    n.vx=(n.tx-n.x)/(d||1)*sp3; n.vy=(n.ty-n.y)/(d||1)*sp3; moveCircle(n,dt);
...
function updateJanitor(dt){
  const J=janitor; J.bob+=dt; J.lineT-=dt; J.noKnock-=dt; J.rage=Math.max(0,J.rage-dt);
  if(J.downT>0){
    J.downT-=dt;
    // flat on his own floor, in his own puddle, twitching
    J.mesh.position.set(J.x,7,J.y);
    J.mesh.rotation.z=-Math.PI/2+Math.sin(J.bob*9)*.03;
    if(J.downT<=0){
      J.mesh.rotation.z=0; J.mesh.position.y=0;
      const M=J.mesh.userData; if(M.mop) M.mop.visible=true;
      J.noKnock=12; J.rage=6;
      say(J,'that was... impolite.');
      sfxHeart(.9);
    }
    return;
  }
  const spd=J.speed + Math.min(150, score/700) + (blackoutT>0?60:0) + (J.rage>0?85:0);
...
function die(){
  ...
  mallet.visible=false; mopVM.visible=false; syrVM.visible=false; pmalletVM.visible=false;
  ...
  const px=-Math.sin(dragDir), pz=Math.cos(dragDir);
  const room=(qx,qy)=>Math.min(qx-WALL,W-WALL-qx,qy-WALL,H-WALL-qy);
  camSide=room(player.x+px*175,player.y+pz*175)>=room(player.x-px*175,player.y-pz*175)?1:-1;
  corpse=new THREE.Group();
  const body=buildPlayerBody(); shadowify(body);
  body.rotation.z=Math.PI/2; body.position.y=10;   // flat on your back, face up
  corpse.add(body); corpse.userData.body=body;
  corpse.rotation.y=-dragDir;                       // feet toward the closet — he drags by the ankles
  corpse.position.set(player.x,0,player.y);
  dyn.add(corpse);
  say(janitor,pick(['fresh mess.','shhh. almost done.','the closet is warm.','you kept me waiting.']));
}
function updateCatch(dt){
  const J=janitor, dx=Math.cos(dragDir), dz=Math.sin(dragDir);
  if(!deathDone && deathT>0.9){
    // the drag: he walks backward, you slide, the floor remembers
    corpse.position.x+=dx*175*dt; corpse.position.z+=dz*175*dt;
    dragStainT-=dt;
    if(dragStainT<=0){ dragStainT=.16; stain(corpse.position.x,corpse.position.z,'#7d1424',11); }
    if(Math.random()<dt*3) sfxStep();
  }
  // he holds station at your ankles
  J.x+=(corpse.position.x+dx*34-J.x)*Math.min(1,dt*8);
  J.y+=(corpse.position.z+dz*34-J.y)*Math.min(1,dt*8);
  J.mesh.position.set(J.x,0,J.y);
  J.mesh.rotation.y=-Math.atan2(corpse.position.z-J.y,corpse.position.x-J.x);
  J.mesh.rotation.z=Math.sin(perf*5)*.05;
  // your last protest: arms flailing at the ceiling
  const B=corpse.userData.body.userData;
  B.armL.up.rotation.z=-.9+Math.sin(perf*9)*.55;
  B.armR.up.rotation.z=.9+Math.sin(perf*9+2)*.55;
  B.armL.lo.rotation.z=Math.sin(perf*11)*.4;
  B.armR.lo.rotation.z=Math.sin(perf*11+1)*.4;
  // the closet accepts its delivery
  if(closetDoor){
    const cd=Math.hypot(J.x-CLOSET.x,J.y-CLOSET.y);
    const tgt=cd<340?1.9:0;
    closetDoor.rotation.y+=(tgt-closetDoor.rotation.y)*Math.min(1,dt*4);
  }
  const arrived=Math.hypot(corpse.position.x-CLOSET.x,corpse.position.z-CLOSET.y)<70;
  if(!deathDone && (arrived||deathT>4.8)){
    deathDone=true;
    fadeEl.style.opacity=1;
    setTimeout(()=>{
      if(closetDoor) closetDoor.rotation.y=0;   // slam
      sfxThud(); sfxCrash(.9);
      finishDeath();
    },560);
  }
...
function finishDeath(){
  ...
  document.getElementById('deathLine').textContent='"'+pick(["all messes end the same way.","you were the mess.","spotless, now.","i mopped where you stood.","the closet has room for one more.","you'll keep. in there."])+'" — the janitor';
...
function syncVisuals(dt){
  ...
  if(state==='play'||state==='title'){
    ...
    // third-person shot: hold the two of them in frame from the roomy side of the drag line
    const bp=corpse?corpse.position:{x:player.x,z:player.y};
    const mx=(bp.x+janitor.x)/2, mz=(bp.z+janitor.y)/2;
    const px=-Math.sin(dragDir)*camSide, pz=Math.cos(dragDir)*camSide;
    const cx=clamp(mx+px*185,WALL+30,W-WALL-30), cz=clamp(mz+pz*185,WALL+30,H-WALL-30);
    camera.position.set(cx,95+Math.sin(perf*1.6)*2.5,cz);
    camera.lookAt(mx,26,mz);
  }
  camera.updateMatrixWorld();
...
if(/[?&]debug=1/.test(location.search)){
  window.__turnover={
    get:()=>({state,score,mopT,knockCount,syrMode,projs:projs.length,boostT:player.boostT,metal:metalOn,pMalletT,
