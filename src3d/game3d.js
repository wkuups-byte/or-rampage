...
function trySwing(){
  ...
  const hasMop=mopT>0;
  const rng=hasMop?185:150, arc=hasMop?0.9:0.62, dmg=hasMop?95:38;
  ...
  for(const p of props){ if(p.dead) continue;
    ...
    if(d<rng){ const a=Math.atan2(py,px); let da=Math.abs(a-player.ang);
      da=Math.min(da,Math.PI*2-da);
      if(da<arc){ hitProp(p,dmg,player.x,player.y,hasMop?1.4:1); hitAny=true;
  ...
  if(hitAny) shake=Math.max(shake,hasMop?5:3.5);
